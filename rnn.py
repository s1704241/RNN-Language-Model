...
class RNN(object):
    ...
    def predict(self, x):
        ...
        for t in range(len(x)):
            net_in = np.dot(self.V, make_Glove(x[t], self.vocab_size)) + np.dot(self.U, s[t - 1])
            s[t] = sigmoid(net_in)
    ...
    def acc_deltas(self, x, d, y, s):
        ...
        for t in reversed(range(len(x))):
            ...
            x_m = make_Glove(x[t], self.vocab_size)
    ...
    def acc_deltas_np(self, x, d, y, s):
        ...
        t = len(x)-1
        x_m = make_Glove(x[-1], self.vocab_size)
    ...
    def acc_deltas_bptt(self, x, d, y, s, steps):
        ...
        for t in reversed(range(len(x))):
            ...
            for m in range(1, steps+1):
                x_n = make_Glove(x[t-m], self.vocab_size)
    ...
    def acc_deltas_bptt_np(self, x, d, y, s, steps):
        ...
        for m in range(1, steps + 1):
            if l-m > 0 :
                x_n = make_Glove(x[l - m], self.vocab_size)
...
if __name__ == "__main__":
    ...
    if mode == "train-lm":
        ...
        # --- your code here --- #
        ##########################
        model = RNN(vocab_size=100, hidden_dims=hdim, out_vocab_size=vocab_size)
        ...
        adjusted_loss = adjusted_loss = adjust_loss(run_loss, fraction_lost, q, mode='basic')
        test_adjusted_loss = adjusted_loss = adjust_loss(test_mean_loss, fraction_lost, q, mode='basic')



        print("Unadjusted on dev: %.03f" % np.exp(run_loss))
        print("Unadjusted on test: %.03f" % np.exp(test_mean_loss))
        print ('the mean loss on dev set is:', mean_loss)
        print ('the mean loss on test set is:', test_mean_loss)
        print("Adjusted for missing vocab on dev: %.03f" % np.exp(adjusted_loss))
        print("Adjusted for missing vocab on test: %.03f" % np.exp(test_adjusted_loss))

        with open('train-lm_result_q4b.txt','a') as f:
            f.write('Using Glove')
            f.write(
                "\nTraining model for {0} epochs\ntraining set: {1} sentences (batch size {2})".format(10, len(X_train),
                                                                                                       100))
            f.write("\nOptimizing loss on {0} sentences".format(len(X_dev)))
            ...
            f.write('\nmean loss on test: {0}'.format(test_mean_loss))
            f.write("\nUnadjusted on dev: %.03f" % np.exp(run_loss))
            f.write("\nAdjusted for missing vocab on dev: %.03f" % np.exp(adjusted_loss))
            f.write("\nUnadjusted on test: %.03f" % np.exp(test_mean_loss))
            f.write("\nAdjusted for missing vocab on test: %.03f" % np.exp(test_adjusted_loss))

        np.save('4bGlove_rnn.U.npy', model.U)
        np.save('4bGlove_rnn.V.npy', model.V)
        np.save('4bGlove_rnn.W.npy', model.W)
    ...
    if mode == "predict-lm":
        ...
        U, V, W = np.load(rnn_folder + "/4bGlove_rnn.U.npy"), np.load(rnn_folder + "/4bGlove_rnn.V.npy"), np.load(rnn_folder + "/4bGlove_rnn.W.npy")
        vocab_size = len(V[0])
        hdim = len(U[0])
